...
void CMRUComboBox::RefreshCtrl()
{
    ...
    for(int i=0;i<m_pMRU->GetSize();i++)
    {
        ...
        {
            // to work with full file path
            // work only with (*m_pMRU)[i] without csBuff
            csBuff = (*m_pMRU)[i];
            pos = csBuff.ReverseFind('\\');
            csBuff = csBuff.Right(csBuff.GetLength()-pos-1);
...
HBRUSH CMRUComboBox::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
