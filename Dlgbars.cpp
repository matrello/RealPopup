...
LRESULT CDlgToolBar::OnIdleUpdateCmdUI(WPARAM wParam, LPARAM) 
{
	if (IsWindowVisible()) 
	{
		...
		if (pParent)
		{
			...
			//SendMessageToDescendants(WM_IDLEUPDATECMDUI, wParam, (LPARAM) 0, TRUE, TRUE);
		}
	}
	return 0L;
}
...
int CDlgStatusBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CDlgStatusBar::SetWindowText(LPCTSTR string)
{

	if (Created)
		CStatusBar::SetWindowText(string);
}
